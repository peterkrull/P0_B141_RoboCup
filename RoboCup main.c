...
const int dir = 1; // sæt 1 for at køre på venstre side af grå streg, 0 for at køre på højre
...
task main()
{
	...
	while (racedone == false) // Main loop til at køre når race endnu ikke er færdig
	{
		...
		line_sensor_val = SensorValue(colorsens);

		//Line tacking function
		if (linetrack == true)
		{
...
task calculatePID()
{
	...
	// brugt til at ændre siden som robotten følger.
	if (dir == 1)
	{
		multi = 1;
	}
